...
sub _dbConnect {
    ...
    if ( $db_user && $db_pass ) {
        if ( $connhash && ref($connhash) eq 'HASH' && exists($connhash->{$dbname}) ) {
            my $dsn = $connhash->{$dbname}->{dsn};
            my $dbh = DBI->connect($dsn, $db_user, $db_pass, {AutoCommit=>1,RaiseError=>1,PrintError=>0} );
            return $dbh if $dbh;
        ...
        return (0, "No DB credentials found for the currently logged in user.");
    }
}
...
sub _checkAcl {
    ...
    if ( @$whitelist_rules ) {
        ...
        return (0, "No whitelist rules defined for $queryop access to $dbname by the currently logged in user.");
    }
}
...
sub _doSelectQuery {
    ...
    if ( $whitelist_rules ) {
        ...
        my ( $selectObj, $err ) = JsonSQL::Query::Select->new($whitelist_rules, $jsonQuery);
        if ( $selectObj ) {
...
sub jsondbgetandsearch {
    ...
    for my $search ( @{ $searchParams } ) {
        ...
        my $topicSearchResult = Foswiki::Func::query( $queryString, undef, { web => $searchWeb } );
        ...
        while ( $topicSearchResult->hasNext ) {
            ...
            for my $matchingRecord ( @{ $queryResult } ) {
                if ( $matchingRecord->{$queryField} eq $formField->{value} ) {
                    if ( $retMany ) {
                        $matchingRecord->{$retKey} = [] if ( not defined $matchingRecord->{$retKey} );
...
sub jsondbinsert {
    ...
    if ( $whitelist_rules ) {
        my ( $insertObj, $err ) = JsonSQL::Query::Insert->new($whitelist_rules, $submittedJson);
        if ( $insertObj ) {
